...
public class addClientes {
       ...
       public void agrega(Cliente cliente){
           //conexion a la base de datos
            ...
            con.conectar();
            //INSERT SQL
            String sql = "INSERT INTO public.\"Cliente\" (nombre,apellidos,dui,nit,sexo,edad,fechaingreso,membresia,nrc,telefono)"+"VALUES(?,?,?,?,?,?,?,?,?,?)";
            String status = "";
           //PREPARANDO SCRIPT PARA SUBIDA
            try(PreparedStatement pst = con.getConexion().prepareStatement(sql)){
                ...
                pst.setInt(9, cliente.nrc);
                pst.setInt(10, cliente.telefono);
       ...
       //metodo para editar  registros en la base de datos
         public void edita(Cliente cliente){
            ...
            "SET  nombre=?, apellidos=?, dui=?, nit=?, sexo=?, edad=?, fechaingreso=?, membresia=?, nrc=?, telefono=?\n" +
